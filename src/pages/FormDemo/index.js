...
const FormDemoPage = () => {
      ...
      render: (text, record) => (
        <Row type="flex" align="middle" className="operation">
          <Button type="link" disabled={record.status}
            onClick={() => pageStore.openModal('edit', record)}>编辑</Button>
          <Divider type="vertical" />
          <Button type="link" disabled={record.status}
            onClick={() => pageStore.delOne(record.id)} loading={pageStore.recordLoding}
          >删除</Button>
        </Row>
      )
    },
  ...
  return (
    <div className="page-form-demo page-content">
      <SearchForm />
      <Divider dashed />
      <Button type="primary" icon="plus" onClick={() => pageStore.openModal('new')}>新建</Button>
      <Table
